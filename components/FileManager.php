...
class FileManager extends CApplicationComponent {
	...
	public function setBasePath($path) {
		if ($path[0] == '/') {
			...
			$this->_basePath = Utils::realpath($path);
		}
		return $this;
	}
	...
	public function getBasePath() {
		if ($this->_basePath === null) {
			$this->_basePath = Utils::realpath(Yii::app()->getBasePath() . '/../' . 'uploads');
		}
	...
	public function getUrlOfDomain($domain) {
		...
		return str_replace(Utils::realpath($base), '', $path);
	}
	...
	public function setThumbBasePath($path) {
		if ($path[0] == '/') {
			$this->_thumbBasePath = Utils::realpath($path);
		}else {
			$this->_thumbBasePath = Utils::realpath(Yii::app()->getBasePath() . '/../' . $path);
		}
		return $this;
	}
	...
	public function getThumbBasePath() {
		if ($this->_thumbBasePath === null) {
			$this->_thumbBasePath = Utils::realpath(Yii::app()->getBasePath() . '/../images');
		}
